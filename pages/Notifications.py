...
from features.analytics.notifications.notification_service import (
    get_notifications, unread_count, create_notification, mark_as_read,
)
...
# ── Split by type ─────────────────────────────────────────────────────────────
ticket_notifs  = [n for n in filtered if "raised"   in n["message"].lower()]
update_notifs  = [n for n in filtered if "updated"  in n["message"].lower()]
high_notifs    = [n for n in filtered if "high"     in n["message"].lower()]
other_notifs   = [n for n in filtered
                  if all(kw not in n["message"].lower()
                         for kw in ("raised","updated","high"))]

tab1, tab2, tab3, tab4 = st.tabs([
    f"🎫 New Tickets ({len(ticket_notifs)})",
    f"🔄 Status Updates ({len(update_notifs)})",
    f"🔴 High Priority ({len(high_notifs)})",
    f"📌 All ({len(filtered)})",
])
...
with tab1:
    if ticket_notifs:
        ...
        st.info("No new ticket notifications.")

with tab2:
    if update_notifs:
        for n in update_notifs:
            notification_item(n["message"], n.get("timestamp", ""),
